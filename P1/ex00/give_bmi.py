...
def check_error(height: list[int | float], weight: list[int | float]):
    ...
    if (len(height) != len(weight)):
        raise SystemExit("AssertionError: lists have to be the same size")
    if not (all(isinstance(x, (int, float)) for x in height) and all(isinstance(x, (int, float)) for x in weight)):
        raise SystemExit("AssertionError: elements must be int or float")
...
def apply_limit(bmi: list[int | float], limit: int)-> list[bool]:
    """
    This function returns a bool list compairing the limit with each member of the bmi list
    """
